...
BOOST_AUTO_TEST_CASE(radius_regression_test)
{
...

    {
        auto results = query.NearestPhantomNodes(
            input, osrm::engine::Approach::UNRESTRICTED, 0.01, std::nullopt, std::nullopt, true);
        BOOST_CHECK_EQUAL(results.size(), 0);
    }

    {
        auto results = query.NearestPhantomNodes(
            input, osrm::engine::Approach::UNRESTRICTED, 1, 0.01, std::nullopt, std::nullopt, true);
        BOOST_CHECK_EQUAL(results.size(), 0);
    }
}
...
BOOST_AUTO_TEST_CASE(permissive_edge_snapping)
{
    ...
    {
        auto results = query.NearestPhantomNodes(
            input, osrm::engine::Approach::UNRESTRICTED, 1000, std::nullopt, std::nullopt, false);
        BOOST_CHECK_EQUAL(results.size(), 1);
    }

    {
        auto results = query.NearestPhantomNodes(
            input, osrm::engine::Approach::UNRESTRICTED, 1000, std::nullopt, std::nullopt, true);
        BOOST_CHECK_EQUAL(results.size(), 2);
    }

    {
        auto results = query.NearestPhantomNodes(
            input, osrm::engine::Approach::UNRESTRICTED, 10, 1000, std::nullopt, std::nullopt, false);
        BOOST_CHECK_EQUAL(results.size(), 1);
    }

    {
        auto results = query.NearestPhantomNodes(
            input, osrm::engine::Approach::UNRESTRICTED, 10, 1000, std::nullopt, std::nullopt, true);
        BOOST_CHECK_EQUAL(results.size(), 2);
    }
}
...
BOOST_AUTO_TEST_CASE(bearing_tests)
{
    ...
    {
        auto results = query.NearestPhantomNodes(
            input, osrm::engine::Approach::UNRESTRICTED, 11000, std::nullopt, std::nullopt, true);
        BOOST_CHECK_EQUAL(results.size(), 2);
    }

    {
        auto results = query.NearestPhantomNodes(
            input, osrm::engine::Approach::UNRESTRICTED, 10, 11000, std::nullopt, std::nullopt, true);
        BOOST_CHECK_EQUAL(results.size(), 2);
    }

    {
        auto results = query.NearestPhantomNodes(
            input, osrm::engine::Approach::UNRESTRICTED, 11000, engine::Bearing{270, 10}, std::nullopt, true);
        BOOST_CHECK_EQUAL(results.size(), 0);
    }

    {
        auto results = query.NearestPhantomNodes(
            input, osrm::engine::Approach::UNRESTRICTED, 10, 11000, engine::Bearing{270, 10}, std::nullopt, true);
        BOOST_CHECK_EQUAL(results.size(), 0);
    }

    {
        auto results = query.NearestPhantomNodes(
            input, osrm::engine::Approach::UNRESTRICTED, 11000, engine::Bearing{45, 10}, std::nullopt, true);
        BOOST_CHECK_EQUAL(results.size(), 2);
    ...
    {
        auto results = query.NearestPhantomNodes(
            input, osrm::engine::Approach::UNRESTRICTED, 10, 11000, engine::Bearing{45, 10}, std::nullopt, true);
        BOOST_CHECK_EQUAL(results.size(), 2);
