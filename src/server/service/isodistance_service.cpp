...
namespace
{
std::string getWrongOptionHelp(const engine::api::IsodistanceParameters &parameters)
{
    std::string help;
...
engine::Status
IsodistanceService::RunQuery(std::size_t prefix_length, std::string &query, ResultT &result)
{
    ...
    auto parameters =
        api::parseParameters<engine::api::IsodistanceParameters>(query_iterator, query.end());
    ...
    return BaseService::routing_machine.Isodistance(*parameters, json_result);
}
}
}
}
